...
const T = {
  it: {
    ...
    activityTitle: "Ultimi arrivi",
    activitySub: "Persone da tutto il mondo stanno ottenendo il loro numero",
    activityJoined: "ha appena partecipato",
    ...
    pendingTitle: "Pagamento ricevuto ✓",
    pendingText: "Il tuo numero sta per arrivare. In rari casi può volerci qualche minuto in più — controlla anche l'email che ti abbiamo scritto, troverai il certificato lì appena pronto.",
  ...
  en: {
    ...
    activityTitle: "Latest arrivals",
    activitySub: "People all over the world are getting their number",
    activityJoined: "just joined",
    ...
    pendingText: "Your number is on its way. In rare cases it may take a little longer — check the email we sent you too, your certificate will be there as soon as it's ready.",
...
// Flag emoji + display name per country code, for the recent activity list.
// Only covers the countries we already map to continents server-side;
// falls back to a generic globe if the country is unknown.
const COUNTRY_INFO = {
  IT: { flag: "🇮🇹", name: "Italia" }, FR: { flag: "🇫🇷", name: "Francia" },
  DE: { flag: "🇩🇪", name: "Germania" }, ES: { flag: "🇪🇸", name: "Spagna" },
  GB: { flag: "🇬🇧", name: "Regno Unito" }, PT: { flag: "🇵🇹", name: "Portogallo" },
  NL: { flag: "🇳🇱", name: "Paesi Bassi" }, BE: { flag: "🇧🇪", name: "Belgio" },
  CH: { flag: "🇨🇭", name: "Svizzera" }, AT: { flag: "🇦🇹", name: "Austria" },
  SE: { flag: "🇸🇪", name: "Svezia" }, NO: { flag: "🇳🇴", name: "Norvegia" },
  DK: { flag: "🇩🇰", name: "Danimarca" }, FI: { flag: "🇫🇮", name: "Finlandia" },
  IE: { flag: "🇮🇪", name: "Irlanda" }, PL: { flag: "🇵🇱", name: "Polonia" },
  GR: { flag: "🇬🇷", name: "Grecia" }, US: { flag: "🇺🇸", name: "Stati Uniti" },
  CA: { flag: "🇨🇦", name: "Canada" }, MX: { flag: "🇲🇽", name: "Messico" },
  BR: { flag: "🇧🇷", name: "Brasile" }, AR: { flag: "🇦🇷", name: "Argentina" },
  CL: { flag: "🇨🇱", name: "Cile" }, CO: { flag: "🇨🇴", name: "Colombia" },
  CN: { flag: "🇨🇳", name: "Cina" }, JP: { flag: "🇯🇵", name: "Giappone" },
  KR: { flag: "🇰🇷", name: "Corea del Sud" }, IN: { flag: "🇮🇳", name: "India" },
  ID: { flag: "🇮🇩", name: "Indonesia" }, PH: { flag: "🇵🇭", name: "Filippine" },
  VN: { flag: "🇻🇳", name: "Vietnam" }, TH: { flag: "🇹🇭", name: "Thailandia" },
  TR: { flag: "🇹🇷", name: "Turchia" }, AE: { flag: "🇦🇪", name: "Emirati Arabi" },
  ZA: { flag: "🇿🇦", name: "Sudafrica" }, NG: { flag: "🇳🇬", name: "Nigeria" },
  EG: { flag: "🇪🇬", name: "Egitto" }, AU: { flag: "🇦🇺", name: "Australia" },
  NZ: { flag: "🇳🇿", name: "Nuova Zelanda" },
};

function getCountryInfo(code) {
  if (code === "XX" || !code) return { flag: "🌍", name: "Da qualche parte" };
  return COUNTRY_INFO[code] || { flag: "🌍", name: code };
}
...
function RecentActivity({ t }) {
  ...
  return (
    <section style={activityStyles.section}>
      <h2 style={activityStyles.title}>{t.activityTitle}</h2>
      <p style={activityStyles.sub}>{t.activitySub}</p>
      <div style={activityStyles.list}>
        {items.map((code, i) => {
          const info = getCountryInfo(code);
          return (
            <div key={i} style={activityStyles.row}>
              <span style={activityStyles.flag}>{info.flag}</span>
              <span style={activityStyles.country}>{info.name}</span>
              <span style={activityStyles.dot}>•</span>
              <span style={activityStyles.time}>{t.activityJoined}</span>
            </div>
          );
        })}
      </div>
...
const activityStyles = {
  section: { width: "100%", marginBottom: 48 },
  title: { fontSize: 20, fontWeight: 800, color: "#EEEEF5", marginBottom: 6, textAlign: "center" },
  sub: { fontSize: 13, color: "#64648A", marginBottom: 20, textAlign: "center" },
  list: { background: "#0B0B14", border: "1px solid #1A1A2E", borderRadius: 16, padding: "6px 0", display: "flex", flexDirection: "column" },
  row: { display: "flex", alignItems: "center", gap: 10, padding: "12px 18px", borderBottom: "1px solid #15151F", fontSize: 14 },
  flag: { fontSize: 18 },
  country: { color: "#EEEEF5", fontWeight: 600, flex: 1, textAlign: "left" },
  dot: { color: "#3A3A50" },
  time: { color: "#64648A", fontSize: 12.5 },
};
...
export default function App() {
  ...
  const [debugMsg, setDebugMsg] = useState(null); // TEMPORARY, remove once fixed
  ...
  useEffect(() => {
    ...
    // SECURITY: the counter, participant row, and certificate email are now
    // created server-side by the Stripe webhook ONLY after a verified payment.
    // This client code no longer increments anything or sends emails — it just
    // reads the current state to show a confirmation screen to the user.
    //
    // NOTE: we intentionally do NOT gate this on an exact pathname match
    // (e.g. "/success"). Stripe's configured redirect URL is managed in the
    // Stripe Dashboard and may include trailing slashes, extra query params,
    // or differ slightly from what we expect. The presence of the saved
    // nick/email in sessionStorage is itself a reliable signal that we just
    // came back from a real checkout redirect — that's all we need to check.
    const savedNick = localStorage.getItem("countone_nick");
    const savedEmail = localStorage.getItem("countone_email");
    const savedAt = localStorage.getItem("countone_saved_at");

    // TEMPORARY DEBUG — shows on screen what we found, to diagnose the
    // mobile redirect issue without needing devtools/screenshots.
    setDebugMsg(
      `nick=${savedNick || "null"} | email=${savedEmail || "null"} | savedAt=${savedAt || "null"} | path=${window.location.pathname}`
    );

    // Ignore stale leftovers older than 30 minutes (e.g. user abandoned checkout)
    const isFresh = savedAt && (Date.now() - parseInt(savedAt, 10)) < 30 * 60 * 1000;

    if (savedNick && savedEmail && isFresh) {
      localStorage.removeItem("countone_nick");
      localStorage.removeItem("countone_email");
      localStorage.removeItem("countone_saved_at");
      setPaying(true);
      window.history.replaceState({}, "", "/");

      let attempts = 0;
      const maxAttempts = 20; // ~20s total, polling every 1s

      const poll = () => {
        attempts++;
        setDebugMsg(`polling... attempt ${attempts}/${maxAttempts}`);
        fetch(`/api/find-certificate?email=${encodeURIComponent(savedEmail)}`)
          .then((r) => r.json())
          .then((data) => {
            setDebugMsg(`attempt ${attempts}: found=${data.found} number=${data.number || "-"}`);
            if (data.found) {
              setCount(data.number);
              setCert({
                number: data.number,
                nick: data.nickname || savedNick,
                email: savedEmail,
              ...
              setPaying(false);
              setPhase("certPending");
              setCert({ nick: savedNick, email: savedEmail });
            }
          })
          .catch((err) => {
            setDebugMsg(`attempt ${attempts}: fetch error - ${err.message}`);
            if (attempts < maxAttempts) setTimeout(poll, 1000);
            else setPaying(false);
          });
      };

      setTimeout(poll, 1200); // small initial delay before first check
    }
  }, []);
  ...
  async function handlePay() {
    ...
    setError(null);
    try {
      // Save nick and email in localStorage (more resilient than sessionStorage
      // across cross-domain redirects on mobile browsers), then redirect to Stripe
      localStorage.setItem("countone_nick", nick.trim());
      localStorage.setItem("countone_email", email.trim());
      localStorage.setItem("countone_saved_at", String(Date.now()));
  ...
  return (
    <div style={s.root}>
      ...
      {/* TEMPORARY DEBUG BANNER — remove once the redirect bug is confirmed fixed */}
      {debugMsg && (
        <div style={{ background: "#1a0a0a", color: "#ff8866", fontSize: 11, padding: "8px 14px", wordBreak: "break-all", borderBottom: "1px solid #442", fontFamily: "monospace" }}>
          🐛 {debugMsg}
        </div>
      )}
      ...
      {/* CERTIFICATE PENDING (webhook slow) */}
      {phase === "certPending" && cert && (
        <main style={s.main}>
          <section style={s.certSection}>
            <div style={s.pendingBox}>
              <div style={s.pendingIcon}>✓</div>
              <h2 style={s.pendingTitle}>{t.pendingTitle}</h2>
              <p style={s.pendingText}>{t.pendingText}</p>
            </div>
            <p style={s.backLink} onClick={reset}>{t.backHome}</p>
          </section>
        </main>
