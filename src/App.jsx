...
const TIME_PHRASES = {
  it: {
    night: "Qualcuno nel mondo sta dormendo. Il numero continua a crescere.",
    morning: "Un nuovo giorno è iniziato. Il numero è già cambiato da quando ti sei svegliato.",
    afternoon: "In questo momento, da qualche parte nel mondo, qualcuno sta ottenendo il suo numero.",
    evening: "La giornata finisce, ma il contatore non si ferma mai.",
  },
  en: {
    night: "Someone in the world is asleep right now. The number keeps growing.",
    morning: "A new day has started. The number has already changed since you woke up.",
    afternoon: "Right now, somewhere in the world, someone is getting their number.",
    evening: "The day is ending, but the counter never stops.",
  },
};

function getTimePhrase(lang) {
  const hour = new Date().getHours();
  let key;
  if (hour >= 0 && hour < 6) key = "night";
  else if (hour >= 6 && hour < 12) key = "morning";
  else if (hour >= 12 && hour < 19) key = "afternoon";
  else key = "evening";
  return TIME_PHRASES[lang][key];
}
      ...
      {phase === "home" && (
        <main style={s.main}>
          <section style={s.hero}>
            ...
            <p style={s.timePhrase}>{getTimePhrase(lang)}</p>
...
const s = {
  ...
  timePhrase: { fontSize: 13, color: "#8A8AAE", fontStyle: "italic", marginBottom: 16, lineHeight: 1.5 },
