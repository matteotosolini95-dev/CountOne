...
const T = {
  it: {
    ...
    lookupTitle: "Quasi fatto! ✓",
    lookupSub: "Inserisci l'email che hai usato per pagare, così ti mostriamo subito il tuo numero.",
    lookupCta: "Trova il mio numero",
    lookupSearching: "Cerco...",
  ...
  en: {
    ...
    lookupTitle: "Almost done! ✓",
    lookupSub: "Enter the email you used to pay, and we'll show you your number right away.",
    lookupCta: "Find my number",
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // not survive the redirect to Stripe's domain and back. Instead, we
    // simply detect that we're on /success and ask the user for the email
    // they just used — we then look up their certificate server-side. This
    // works reliably regardless of browser storage behavior.
  ...
  function lookupCertificate(lookupEmail) {
    ...
    const poll = () => {
      attempts++;
      fetch(`/api/find-certificate?email=${encodeURIComponent(lookupEmail.trim())}`)
        .then((r) => r.json())
        .then((data) => {
          if (data.found) {
            ...
            setCert({ nick: "", email: lookupEmail.trim() });
          }
        })
        .catch(() => {
  ...
  return (
    <div style={s.root}>
      ...
      {/* LOOKUP — after returning from Stripe, ask for the email used to pay */}
      {phase === "lookup" && (
        <main style={s.main}>
          <section style={s.formSection}>
            <h2 style={s.formTitle}>{t.lookupTitle}</h2>
            <p style={s.formSub}>{t.lookupSub}</p>
            <div style={s.formCard}>
              ...
              <button
                style={{
                  ...
                  marginTop: 28,
                  width: "100%",
                  opacity: (!email.trim() || paying) ? 0.4 : 1,
                }}
                onClick={() => lookupCertificate(email)}
                disabled={!email.trim() || paying}
              >
                {paying ? t.lookupSearching : t.lookupCta}
              </button>
            </div>
