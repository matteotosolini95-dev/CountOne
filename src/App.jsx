...
const T = {
  it: {
    ...
    mapTitle: "Da dove arrivano",
    mapSub: "Persone da tutto il mondo stanno ottenendo il loro numero",
    mapCount: "{n} partecipanti recenti sulla mappa",
    mapEmpty: "Sii il primo a comparire sulla mappa",
  ...
  en: {
    ...
    mapTitle: "Where they're from",
    mapSub: "People all over the world are getting their number",
    mapCount: "{n} recent participants on the map",
    mapEmpty: "Be the first to show up on the map",
...
// Approximate bounding boxes (in a 1000x500 viewBox) used to scatter dots
// inside the right region — purely illustrative, not real geolocation.
const CONTINENT_BOX = {
  europe: { x: 460, y: 85, w: 100, h: 80 },
  northAmerica: { x: 120, y: 60, w: 200, h: 130 },
  southAmerica: { x: 260, y: 240, w: 100, h: 150 },
  asia: { x: 590, y: 70, w: 270, h: 160 },
  africa: { x: 460, y: 180, w: 130, h: 170 },
  oceania: { x: 790, y: 300, w: 110, h: 80 },
  unknown: { x: 480, y: 460, w: 40, h: 20 },
};

// A small set of seed dots so the map never looks empty before real traffic
// arrives. They are visually identical to real dots and get mixed in.
const SEED_DOTS = [
  { c: "europe", x: 0.2, y: 0.3 }, { c: "europe", x: 0.5, y: 0.6 }, { c: "europe", x: 0.7, y: 0.2 },
  { c: "europe", x: 0.35, y: 0.8 }, { c: "europe", x: 0.85, y: 0.5 },
  { c: "northAmerica", x: 0.25, y: 0.35 }, { c: "northAmerica", x: 0.55, y: 0.55 },
  { c: "northAmerica", x: 0.7, y: 0.25 }, { c: "northAmerica", x: 0.4, y: 0.75 },
  { c: "southAmerica", x: 0.4, y: 0.25 }, { c: "southAmerica", x: 0.6, y: 0.5 },
  { c: "southAmerica", x: 0.3, y: 0.7 },
  { c: "asia", x: 0.15, y: 0.3 }, { c: "asia", x: 0.4, y: 0.55 }, { c: "asia", x: 0.65, y: 0.2 },
  { c: "asia", x: 0.8, y: 0.45 }, { c: "asia", x: 0.5, y: 0.8 }, { c: "asia", x: 0.25, y: 0.7 },
  { c: "africa", x: 0.3, y: 0.2 }, { c: "africa", x: 0.55, y: 0.45 }, { c: "africa", x: 0.4, y: 0.75 },
  { c: "oceania", x: 0.3, y: 0.35 }, { c: "oceania", x: 0.6, y: 0.6 },
  { c: "northAmerica", x: 0.15, y: 0.6 }, { c: "europe", x: 0.55, y: 0.4 },
];

function seededOffset(seed, range) {
  // simple deterministic pseudo-random based on a seed string, so dots don't
...
function WorldMap({ t }) {
  ...
  useEffect(() => {
    fetch("/api/map-data")
      .then((r) => r.json())
      .then((data) => setContinents(data.continents || []))
      .catch(() => setContinents([]));
  }, []);

  const realDots = continents.map((c, i) => {
    const box = CONTINENT_BOX[c] || CONTINENT_BOX.unknown;
    const ox = seededOffset(`${c}-${i}-x`, box.w);
    const oy = seededOffset(`${c}-${i}-y`, box.h);
    return { x: box.x + ox, y: box.y + oy, key: `real-${c}-${i}`, real: true };
  });

  const seedDots = SEED_DOTS.map((d, i) => {
    const box = CONTINENT_BOX[d.c];
    return { x: box.x + d.x * box.w, y: box.y + d.y * box.h, key: `seed-${i}`, real: false };
  });

  const dots = [...seedDots, ...realDots];

  return (
    <section style={mapStyles.section}>
      <h2 style={mapStyles.title}>{t.mapTitle}</h2>
      <p style={mapStyles.sub}>{t.mapSub}</p>
      <div style={mapStyles.frame}>
        <svg viewBox="0 0 1000 500" style={mapStyles.svg} preserveAspectRatio="xMidYMid meet">
          {/* Stylized continent shapes — illustrative, not cartographically exact */}
          <g fill="#16161F" stroke="#2A2A40" strokeWidth="1.2">
            {/* North America */}
            <path d="M 130,75 Q 180,55 250,65 Q 300,75 310,110 Q 320,140 290,170 Q 260,195 220,190 Q 180,200 160,170 Q 130,150 125,115 Z" />
            {/* South America */}
            <path d="M 290,250 Q 320,245 335,270 Q 350,310 345,360 Q 340,395 320,400 Q 295,395 285,360 Q 270,310 275,275 Z" />
            {/* Europe */}
            <path d="M 460,90 Q 500,80 545,90 Q 565,105 555,130 Q 545,155 510,165 Q 480,160 465,135 Q 455,110 460,90 Z" />
            {/* Africa */}
            <path d="M 475,185 Q 520,175 555,195 Q 575,220 565,260 Q 560,310 535,345 Q 510,350 495,320 Q 475,280 470,235 Z" />
            {/* Asia */}
            <path d="M 595,70 Q 680,55 760,75 Q 840,90 855,135 Q 860,175 810,200 Q 750,215 690,195 Q 630,180 600,140 Q 585,105 595,70 Z" />
            {/* Oceania */}
            <path d="M 800,310 Q 840,300 880,315 Q 895,335 875,355 Q 845,365 815,350 Q 795,335 800,310 Z" />
          </g>
          {dots.map((d) => (
            <circle key={d.key} cx={d.x} cy={d.y} r={d.real ? 4.5 : 3.5} fill="#F5A623" opacity={d.real ? 0.9 : 0.55}>
              <animate attributeName="opacity" values={d.real ? "0.5;1;0.5" : "0.3;0.6;0.3"} dur={d.real ? "2.2s" : "3.6s"} repeatCount="indefinite" />
            </circle>
          ))}
        </svg>
      </div>
      <p style={mapStyles.count}>
        {continents.length > 0
          ? t.mapCount.replace("{n}", continents.length)
          : t.mapEmpty}
      </p>
    </section>
  );
...
const mapStyles = {
  section: { width: "100%", marginBottom: 48, textAlign: "center" },
  title: { fontSize: 20, fontWeight: 800, color: "#EEEEF5", marginBottom: 6 },
  sub: { fontSize: 13, color: "#64648A", marginBottom: 20 },
  frame: { background: "#0B0B14", border: "1px solid #1A1A2E", borderRadius: 16, padding: "12px 8px", overflow: "hidden" },
  svg: { width: "100%", height: "auto", display: "block" },
  count: { fontSize: 12, color: "#64648A", marginTop: 14 },
};
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (window.location.pathname === "/success" || params.get("payment") === "success") {
      ...
      const savedCountBefore = sessionStorage.getItem("countone_count_before");
      if (savedNick && savedEmail) {
        sessionStorage.removeItem("countone_nick");
        sessionStorage.removeItem("countone_email");
        sessionStorage.removeItem("countone_count_before");
        setPaying(true);
        window.history.replaceState({}, "", "/");

        const beforeCount = savedCountBefore ? parseInt(savedCountBefore, 10) : null;
        let attempts = 0;
        const maxAttempts = 15; // ~15s total, polling every 1s

        const poll = () => {
          attempts++;
          getCount()
            .then((freshCount) => {
              const hasIncreased = beforeCount === null || freshCount > beforeCount;
              if (hasIncreased) {
                ...
                // Webhook took too long — payment likely still succeeded
                // (the email will arrive), but we stop polling to avoid
                // hanging forever. Show a reassuring message instead of an error.
                setPaying(false);
  ...
  async function handlePay() {
    ...
    try {
      ...
      sessionStorage.setItem("countone_count_before", String(count ?? ""));
