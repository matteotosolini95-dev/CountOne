...
async function saveParticipant(number, nickname, email, continent) {
  const SUPABASE_URL = process.env.SUPABASE_URL;
  const SUPABASE_KEY = process.env.SUPABASE_SERVICE_KEY;
  ...
  await fetch(`${SUPABASE_URL}/rest/v1/participants`, {
    ...
    body: JSON.stringify({ number, nickname, email, continent }),
  });
}
...
export default async function handler(req, res) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    try {
      ...
      await saveParticipant(newCount, nick, email, continent);
      if (email) {
