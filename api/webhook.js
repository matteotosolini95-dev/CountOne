...
async function saveParticipant(number, nickname, email, continent, country) {
  const SUPABASE_URL = process.env.SUPABASE_URL;
  const SUPABASE_KEY = process.env.SUPABASE_SERVICE_KEY;
  ...
  await fetch(`${SUPABASE_URL}/rest/v1/participants`, {
    ...
    body: JSON.stringify({ number, nickname, email, continent, country }),
  });
}

// Maps an ISO country code (from Vercel's geo header) to a continent key.
// Approximate on purpose — privacy-friendly, no precise location ever stored.
function countryToContinent(code) {
  if (!code) return "unknown";
  const c = code.toUpperCase();
  const europe = ["IT","FR","DE","ES","GB","PT","NL","BE","CH","AT","SE","NO","DK","FI","IE","PL","GR","CZ","RO","HU","BG","HR","SK","SI","LT","LV","EE","LU","MT","CY","IS"];
  const northAmerica = ["US","CA","MX"];
  const southAmerica = ["BR","AR","CL","CO","PE","VE","EC","BO","PY","UY"];
  const asia = ["CN","JP","KR","IN","ID","PH","VN","TH","MY","SG","PK","BD","TR","SA","AE","IL","HK","TW"];
  const africa = ["ZA","NG","EG","KE","MA","GH","DZ","TN","ET"];
  const oceania = ["AU","NZ","FJ","PG"];

  if (europe.includes(c)) return "europe";
  if (northAmerica.includes(c)) return "northAmerica";
  if (southAmerica.includes(c)) return "southAmerica";
...
function getBadge(number) {
  if (number <= 100) return { emoji: "🥇", label: "Pioniere", color: "#F5A623" };
  if (number <= 1000) return { emoji: "🥈", label: "Veterano", color: "#C9CDD6" };
  if (number <= 10000) return { emoji: "🥉", label: "Membro", color: "#C8895A" };
  return { emoji: "⚪", label: "Partecipante", color: "#9A9AB5" };
}
...
async function sendEmail(email, nick, number, date, continent) {
  const RESEND_KEY = process.env.RESEND_API_KEY;
  const badge = getBadge(number);
  const continentLabel = CONTINENT_LABEL[continent] || null;
...
<body style="background:#07070E;color:#EEEEF5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;margin:0;padding:48px 20px;">
  <div style="max-width:480px;margin:0 auto;">
    ...
    <div style="background:linear-gradient(160deg,#13131F,#0A0A12);border:1px solid #F5A62340;border-radius:24px;padding:36px 30px;box-shadow:0 20px 60px -20px #00000080;">
      ...
      <div style="text-align:center;margin-bottom:30px;">
        <span style="display:inline-flex;align-items:center;gap:6px;font-size:13px;font-weight:700;color:${badge.color};background:${badge.color}18;border:1px solid ${badge.color}55;border-radius:20px;padding:6px 16px;">
      ...
      <div style="border-top:1px solid #1E1E2E;padding-top:20px;">
        ...
        ${continentLabel ? `
        <div style="display:flex;justify-content:space-between;padding:9px 0;">
          <span style="color:#64648A;font-size:13px;">Da</span>
          <span style="font-weight:600;font-size:13px;color:#EEEEF5;">${continentLabel}</span>
        </div>` : ""}
        <div style="display:flex;justify-content:space-between;padding:9px 0;">
    ...
    <div style="text-align:center;margin-top:28px;">
      <a href="https://countone.org" style="display:inline-block;background:#F5A623;color:#07070E;font-weight:800;font-size:14px;text-decoration:none;border-radius:12px;padding:13px 28px;">
        Ottieni un altro numero →
      </a>
      <p style="color:#45455A;font-size:11.5px;margin-top:10px;line-height:1.5;">
        Ogni certificato ha un numero diverso. Puoi collezionarne quanti vuoi.
      </p>
    </div>

    <p style="color:#45455A;font-size:11px;text-align:center;margin-top:32px;line-height:1.6;">
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: email,
      subject: `${badge.emoji} Il tuo certificato CountOne — Numero #${number}`,
      html,
    }),
  });
}
...
export default async function handler(req, res) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    // DEBUG: log exactly what Stripe sent us, visible in Vercel function logs,
    // to diagnose cases where email/nick/country come back empty.
    ...
    try {
      ...
      await saveParticipant(newCount, nick, email, continent, countryCode);
      if (email) {
        await sendEmail(email, nick, newCount, date, continent);
      }
    } catch (err) {
